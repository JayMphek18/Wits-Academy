...
<body class="body">
    ...
    <?php
            ...
            while($quiz=$result1->fetch_assoc())
        {
            ...
            $quizid=$quiz['quiz_id'];
            /*
            $sql="select * from attempts where stud_num='$userid' and quiz_id='$quizid'";
            $result1=$conn->query($sql); 
            $rowcount= mysqli_num_rows($result1);*/
    ?>
   <!--For new quizzes -->
   <li class="list-item">
        <div class="text-block"><?php echo $qname ?></div>
            <a href="../php/recordAttempt.php?id=<?php echo $quiz['course_id']?>&quiz=<?php echo $quizid ?>&quiz_name=<?php echo $qname?>&num=<?php echo $quiz['ques_num']?>"
                class="btn btn-primary"><i class="fas fa-pencil"></i> Attempt Quiz</a>
        </li>
    <?php }}else{?>
                <li class="list item">
                    <div class="text-block">No new quizzes available</div>
                </li>
           
    <?php } ?>
            <li class="list-item">
                    <a href="pastQuizzes.php?id=<?php echo $id ?>" class="btn btn-success">View graded quizzes <i class="fas fa-long-arrow-right"></i></a>
                </li>
            </ul>
        </section>
