...
<body class="body">
        ...
        <ul>
            ...
            <form class="form" action="../php/update_ans.php?id=<?php echo $id ?>&quiz_id=<?php echo $quiz?>&q=<?php echo $q ?>&num=<?php echo $num?>&qname=<?php echo $name?>" method="post">
                ...
                <?php
                    $sql1="select * from answers where ques_id='$q'";
                    $result1=$conn->query($sql1);
                    $i=1;
                    while($answer=$result1->fetch_assoc()){
                ?>
                <input type="text"  name="value_<?php echo $i ?>" maxlength="255" value="<?php echo $answer['answer_text'] ?>" class="input" required=""></br>
                <?php  ++$i; } ?>
                ...
                <input type="submit"  class="submit" name="submit" value="Update Answers" />
            </form>
        </div>
