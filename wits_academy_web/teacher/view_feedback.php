...
<body class="body">
    <?php
        ...
        $sql="select * from feedback where course_id='$id'";
        $result1=$conn->query($sql);
    ?>   
    <section class="section3">
        <h2 class="heading">Course Feedback</h2>
        <ul role="list" class="list-5">
        <?php 
            if(mysqli_num_rows($result1) == 0){ ?>
                <li class="list-item-5"><div> No course feedbacks available </div></li>
            <?php }else{
                while($feedback=$result1->fetch_assoc())
                {
                    $user=getProfile($feedback['user'], $conn);
                    
            ?>
            <li class="list-item-5"><div class="link-14"><i class="fas fa-star"></i> <?php echo $feedback['course_rate'] ?></div>
            <div class="text-block-12">Reviewed by <?php echo $user['first_name']." ".$user['last_name'] ?></div>
            <a href="viewFeedback.php?id=<?php echo $id ?>&feedback=<?php echo $feedback['feedback_id'] ?>" class="button-13 w-button"><i class="fas fa-eye"></i> View Feedback</a>
            </li>
            <?php } }?>
        </ul>
            </section>
